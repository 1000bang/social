...
export function SimpleBarChart({ data, scrollable = false }: { data: ChartBucket[]; scrollable?: boolean }) {
	const max = Math.max(...data.map((d) => d.count), 1);
	const labelStep = scrollable ? 1 : Math.ceil(data.length / MAX_VISIBLE_LABELS);

	return (
		<div className={`simple-bar-chart${scrollable ? " simple-bar-chart--scrollable" : ""}`}>
			{data.map((d, index) => (
